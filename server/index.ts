...
type Progress = { playerId: string; xp: number; coins: number; completed: number[]; dailyClaimed: string; lastActiveDay: string; streak: number }
...
const validProgress = (value: unknown): value is Progress => {
  ...
  return /^[a-zA-Z0-9-]{16,80}$/.test(item.playerId) && Number.isInteger(item.xp) && item.xp >= 0 && Number.isInteger(item.coins) && item.coins >= 0 && Array.isArray(item.completed) && item.completed.every((id) => Number.isInteger(id) && id > 0 && id <= 99) && typeof item.dailyClaimed === 'string' && /^$|^\d{4}-\d{2}-\d{2}$/.test(item.dailyClaimed) && typeof item.lastActiveDay === 'string' && /^$|^\d{4}-\d{2}-\d{2}$/.test(item.lastActiveDay) && Number.isInteger(item.streak) && item.streak >= 0 && item.streak <= 9999
}
...
createServer(async (request, response) => {
  try {
    ...
    if (match && request.method === 'GET') {
      ...
      const result = await pool.query('SELECT player_id AS "playerId", xp, coins, completed, daily_claimed AS "dailyClaimed", last_active_day AS "lastActiveDay", streak FROM game_profiles WHERE player_id = $1', [match[1]])
    ...
    if (path === '/api/progress' && request.method === 'PUT') {
      ...
      if (!databaseReady || !pool) return respondJson(response, 503, { error: 'Persistência ainda não configurada.' })
      const result = await pool.query(`INSERT INTO game_profiles (player_id, xp, coins, completed, daily_claimed, last_active_day, streak)
        VALUES ($1, $2, $3, $4::jsonb, $5, $6, $7)
        ON CONFLICT (player_id) DO UPDATE SET xp = GREATEST(game_profiles.xp, EXCLUDED.xp), coins = GREATEST(game_profiles.coins, EXCLUDED.coins), completed = EXCLUDED.completed, daily_claimed = EXCLUDED.daily_claimed, last_active_day = EXCLUDED.last_active_day, streak = GREATEST(game_profiles.streak, EXCLUDED.streak), updated_at = NOW()
        RETURNING player_id AS "playerId", xp, coins, completed, daily_claimed AS "dailyClaimed", last_active_day AS "lastActiveDay", streak`, [progress.playerId, progress.xp, progress.coins, JSON.stringify([...new Set(progress.completed)].sort()), progress.dailyClaimed, progress.lastActiveDay, progress.streak])
